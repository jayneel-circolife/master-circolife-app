...
class _DevicesScreenState extends State<DevicesScreen> {
  ...
  Future<void> pingAllDevices() async {
    for (var device in deviceLookup.values) {
      final cmdIn = "${device.deviceid}/cmdin";
      final cmdOut = "${device.deviceid}/cmdout";

      mqttClient.subscribe(cmdOut, MqttQos.atLeastOnce);

      final builder = MqttClientPayloadBuilder();
      builder.addString("Hello ESP");

      mqttClient.publishMessage(cmdIn, MqttQos.atLeastOnce, builder.payload!);
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...ownedDevices.map((device) => ListTile(
            leading: Icon(Icons.wifi, color: device.isOnline ? Colors.green : Colors.grey),
            title: Text(device.deviceName.toString()),
            subtitle: Text(device.deviceid.toString()),
